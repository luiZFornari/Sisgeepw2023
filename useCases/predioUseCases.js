...
const addPredioDB = async (body) => {
    try {
        ...
        const results = await pool.query(`INSERT INTO predios (nome, descricao,
            sigla) VALUES ($1, $2, $3) 
...
const updatePredioDB = async (body) => {
    try {
        ...
        const results = await pool.query(`UPDATE predios SET nome=$1,
...
const deletePredioDB = async (codigo) => {
    try {        
        const results = await pool.query(`DELETE FROM predios 
...
const getPredioPorCodigoDB = async (codigo) => {
    try {        
        const results = await pool.query(`SELECT * FROM predios 
        WHERE codigo = $1`, [codigo]);
        if (results.rowCount == 0){
            throw `Nenhum registro encontrado com o código ${codigo}`
        } else {
            const predio = results.rows[0];
            return new Predio(predio.codigo, predio.nome,
                 predio.descricao, predio.sigla);
        }
    } catch (err){
...
module.exports = { getPrediosDB, addPredioDB, 
    updatePredioDB, deletePredioDB, getPredioPorCodigoDB }